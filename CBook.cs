	...
	class CBook
	{
				...
				using (BinaryReader reader = new BinaryReader(fs))
				{
					...
					else
					{
						while (reader.BaseStream.Position != reader.BaseStream.Length)
						{
							...
							ushort g = ReadUInt16(reader);
							CRec rec = new CRec(MbwToTnt(m, b, w)) { games = g };
							recList.Add(rec);
						}
					}
				}
		...
		public int AddUciMate(string[] moves)
		{
			...
			for (int n = 0; n < moves.Length; n++)
			{
				...
				if (recList.AddRec(rec, (n & 1) != (moves.Length & 1)))
					ca++;
		...
		public string GetMove(string fen, string moves, int rnd, ref bool bookWrite)
		{
			...
			if (bst.rec != null)
				Console.WriteLine($"info string book {umo} games {bst.rec.games} possible {emoList.Count}");
			return umo;
		}
			...
			else
			{
				...
				else
				{
					...
					foreach (CEmo e in el)
					{
						...
						Console.WriteLine(String.Format("{0,2} {1,-4} {2,6}", i++, umo, e.rec.games));
					}
				}
			}
		}
